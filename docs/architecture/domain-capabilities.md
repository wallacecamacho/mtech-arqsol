...
| Consultar lançamentos por data | `GET /api/entries?date=&page=&pageSize=` — lista paginada (max 100 por página) com headers `X-Total-Count` | Implementado | P0 |
| Publicar evento de integração | Emite `EntryCreatedIntegrationEvent` via RabbitMQ (Outbox) | Implementado | P0 |
| Garantia de entrega (Outbox) | Salvar evento no banco antes de publicar; retry via background service; dead-letter após 10 retries | Implementado | P0 |
| Consulta por período | `GET /api/entries?from=&to=` — range de datas | ~ Parcial (repository tem método, sem endpoint) | P1 |
| Cancelamento de lançamento | Marcar lançamento como cancelado (sem deleção física) | - Não implementado | P2 |
| Paginação de resultados | Page/pageSize com max 100 — `X-Total-Count` no header | Implementado | P2 |

### Contexto: Consolidação

| Capacidade | Descrição | Status | Prioridade |
|---|---|---|---|
| Calcular saldo diário | Consumir `EntryCreatedIntegrationEvent`, upsert atômico `ON CONFLICT DO UPDATE` (sem race condition) | Implementado | P0 |
| Idempotência do consumer | Tabela `processed_events(entry_id PK)` — reentregas são detectadas e ignoradas | Implementado | P0 |
...
| Observabilidade (traces, logs, **métricas**) | OpenTelemetry Traces + Metrics (ASP.NET Core + Runtime) + Jaeger + Seq | P1 |
| CI automático (build + testes) | GitHub Actions — build + unit + integration + E2E | P1 |
| Health checks (live/ready) | Todos os serviços | P1 |
| Load test automatizado | k6 multi-cenário (`tests/load/k6-scenarios.js`) | P1 |
...
| E2E tests automatizados | `CashFlow.E2E.Tests` — Entries → Outbox → RabbitMQ → Consolidated | P1 |
| IdP real (Keycloak / Entra ID) | - AuthController é demo (PBKDF2, disclaimer explícito) | P2 |
